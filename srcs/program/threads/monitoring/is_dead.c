...
/*   Updated: 2025/04/23 14:08:19 by aaljazza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	philo_dead(t_philo *philos, size_t time_to_die)
{
	...
	if (ft_current_time () - philos->last_meal >= time_to_die
		&& philos->eating == 0)
...
// dead lock is one lock for all philos 
// so we just use philos[0] to lock and unlock it.
...
int	is_dead(t_philo *philos)
{
	...
	while (i < philos[0].num_of_philos)
	{
		...
		{
			print_msg ("die", &philos[i], philos[i].id);
